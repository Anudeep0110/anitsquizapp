...
const SQuiz = () => {
    ...
    const handleTimeOver = () => {
        axios.post('https://anitsquizapp.onrender.com/updatemarks',{quizid:quizid,uname:uname,marks:(ch === qs[ind].answer)?(marks+1)*mpq:(marks)*mpq,subId:subId})
            .then(res => {
    ...
    React.useEffect(() => {
        ...
        axios.post('https://anitsquizapp.onrender.com/quizquestions',{quizid:quizid})
        .then((res) => {
            setTimeout(() => {
                setLoading(false)
            },2000)
